...
#    region = "OUS"
    try:
        ...
        if region == "US":
            ...
            # )
        # endregion
        print("DB connection to ", region, " staging.")
        return conn

    except:
        print("NO db connection to ", region, " region.")
        raise
            ...
            if jsonStr != None:
                ...
                if valid:
                    if row_counter >= int(start_record) and row_counter < int(record_count):
                        # determine valid time order (descending from most recent to least recent)
